        ...
        for i in range(3):
            for j in range(2):
                ...
                for id in temp_ing["ids"].keys():
                    ...
                    stats_dic[id]["min"] = stats_dic[id]["min"] + (np.floor(temp_ing["ids"][id]["minimum"] * ((1 + (temp_boost_array[i,j]/100)) if temp_boost_array[i,j]>0 else (-1 + (temp_boost_array[i,j]/100)))))  
                    stats_dic[id]["min"] = stats_dic[id]["min"] + (np.floor(temp_ing["ids"][id]["maximum"] * ((1 + (temp_boost_array[i,j]/100)) if temp_boost_array[i,j]>0 else (-1 + (temp_boost_array[i,j]/100)))))  

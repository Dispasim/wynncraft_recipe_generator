...
class Individual:
    ...
    def encode_craft(self, CRAFTER_ENC,FIRST_INGREDIENT_QUALITY,SECOND_INGREDIENT_QUALITY,attack_speed="SLOW"):
        ...
        # Recipe
        craft_vec.append(
            get_recipe_id(self.item,self.lvl_max),
            CRAFTER_ENC["RECIPE_ID_BITLEN"]
        )


        # Material tiers
        for mat_tier in [FIRST_INGREDIENT_QUALITY,SECOND_INGREDIENT_QUALITY]:
            craft_vec.append(
                mat_tier - 1,
        ...
        if self.item in  [
                    ...
                    "bow"]:
            craft_vec.append(
                CRAFTER_ENC["CRAFTED_ATK_SPD"][attack_speed],
    ...
    def encode_wynn_craft_hash(self,ingredient_ids, material_tiers, attack_speed="SLOW", is_weapon=False):
        bits = []
        ...
        if len(ingredient_ids) != 6:
            raise ValueError(f"Un hash WynnCrafter doit avoir exactement 6 ingrédients, reçu {len(ingredient_ids)}")

        for ingredient_id in ingredient_ids:
            self.wb_append_bits_little(bits, int(ingredient_id), CRAFTER_ENC["ING_ID_BITLEN"])

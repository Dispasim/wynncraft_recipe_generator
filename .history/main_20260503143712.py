...
print(f"Nombre d'ingrédients : {print(ing["name"] for ing in data)}")
raise ValueError
#%% Génétique
...
def sus_selection(population, n_select):
    total_fitness = sum(ind.fitness(TRANSLATED_STAT,duration_min = DURATION_MIN, min_max_or_mean = MIN_MAX_OR_MEAN) for ind in population)
    ...
    for ind in population:
        current += ind.fitness(TRANSLATED_STAT,duration_min = DURATION_MIN, min_max_or_mean = MIN_MAX_OR_MEAN)
...
def tournament_selection(population, n_select, k=TOURNAMENT_SELECTION_ROUND):
    ...
    for _ in range(n_select):
        ...
        winner = max(
            tournament,
            key=lambda ind: ind.fitness(TRANSLATED_STAT, duration_min=DURATION_MIN, min_max_or_mean=MIN_MAX_OR_MEAN)
        )
...
def genetic_algorithm():
    ...
    for generation in tqdm(range(GENERATIONS),desc="Processing..."):
        ...
        best = max(population, key=lambda ind: ind.fitness(TRANSLATED_STAT,duration_min = DURATION_MIN, min_max_or_mean = MIN_MAX_OR_MEAN))
        if best.fitness(TRANSLATED_STAT,duration_min = DURATION_MIN, min_max_or_mean = MIN_MAX_OR_MEAN)>best_total[1]:
            best_total = (best,best.fitness(TRANSLATED_STAT,duration_min = DURATION_MIN, min_max_or_mean = MIN_MAX_OR_MEAN))
        print(f"Génération {generation} | Best fitness: {best.fitness(TRANSLATED_STAT,duration_min = DURATION_MIN, min_max_or_mean = MIN_MAX_OR_MEAN)}")
                ...
                for ing4 in data:
                    for ing5 in data:
                        for ing6 in data:
                            ...
                            ind.recalculate_duration()
                            #fitness = ind.fitness(TRANSLATED_STAT,duration_min = DURATION_MIN, min_max_or_mean = MIN_MAX_OR_MEAN)
                            if ind.fitness(TRANSLATED_STAT,duration_min = DURATION_MIN, min_max_or_mean = MIN_MAX_OR_MEAN) > best_total[1]:
                                best_total = (ind,ind.fitness(TRANSLATED_STAT,duration_min = DURATION_MIN, min_max_or_mean = MIN_MAX_OR_MEAN))
    return best_total
...
if __name__ == "__main__":
    ...
    print(f"Meilleur score : {best_solution[0].fitness(TRANSLATED_STAT,duration_min = DURATION_MIN, min_max_or_mean = MIN_MAX_OR_MEAN)}")

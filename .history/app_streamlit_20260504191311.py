    ...
    if run:
        ...
        for gen_idx in range(generations):
            ...
            if best.fitness(translated_stat,duration_min = duration_min, min_max_or_mean = min_max_or_mean, req_stats = st.session_state.req_stats)>best_total[1]:
                best_total = (best,best.fitness(translated_stat,duration_min = duration_min, min_max_or_mean = min_max_or_mean, req_stats = st.session_state.req_stats))
        ...
        st.write("Best fitness:", best[1])
        st.write(f"Hash : CR-{best[0].encode_wynn_craft_hash([quality_one,quality_two])}")
        st.write(f"lien : https://wynnbuilder-beta.github.io/crafter/#{best[0].encode_wynn_craft_hash([quality_one,quality_two])}")
        st.iframe(f"https://wynnbuilder-beta.github.io/crafter/#{best[0].encode_wynn_craft_hash([quality_one,quality_two])}", height=1700)

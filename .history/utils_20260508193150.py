...
def calculate_ingredient_quality_bonus(item,q_ing1,q_ing2):
    ...
    if item == "helmet" or item == "boots" or item == "wand" or item == "spear" or item == "potion" or item == "bracelet":
        if q_ing1 == 1 and q_ing2 == 1:
            ...
            rep = 1.40    
    elif item == "chestplate" or item == "leggings" or item == "bow" or item == "relik" or item == "dagger" or item == "food" or item == "necklace":
        if q_ing1 == 1 and q_ing2 == 1:
            rep = 1
        elif q_ing1 == 2 and q_ing2 == 1:
            rep = 1.1666
...
def remove_useless_ingredients(ingredients,stats,item):
    if item in ["chestplate",
                    "helmet",
                    "leggings",
                    "boots",
        ...
        return [
            ing for ing in ingredients
            if (any(stat in ing["ids"].keys() for stat in stats)) or (ing["itemIDs"]["dura"]>0) or (any(x != 0 for x in (ing["posMods"].values()) ))
        ]
    elif item in ["potion",
                    "scroll",
                    "food"]:
        return [
            ing for ing in ingredients
            if (any(stat in ing["ids"].keys() for stat in stats)) or (ing["consumableIDs"]["dura"]>0) or (any(x > 0 for x in (ing["posMods"].values()) ))
        ]
...
"""def update_best_archive(archive, population, x, fitness_fn):
    seen = set(ind for ind, _ in archive)
...
def update_best_archive(archive, population, x, fitness_fn, hash_fn):
    """
    archive : [(ind, fitness, hash)]
    population : population actuelle
    x : nombre max d'éléments
    fitness_fn : fonction fitness
    hash_fn : fonction qui retourne un identifiant unique (string)
    """

    # 🔹 set des hashes déjà présents
    existing_hashes = {h for _, _, h in archive}
    ...
    for ind in population:
        ...
        archive.append((copy.deepcopy(ind), fit, h))
...
def update_best_archive_fast(archive_dict, population, x, fitness_fn, hash_fn):
    """
    archive_dict : dict {hash: (fitness, individual)}
    """

    for ind in population:
        h = hash_fn(ind)

        # ⚡ calcule fitness UNE seule fois
        fit = fitness_fn(ind)

        # 🔹 nouveau ou meilleur
        if h not in archive_dict or fit > archive_dict[h][0]:
            archive_dict[h] = (fit, ind)

    # 🔹 si trop d'éléments → on coupe
    if len(archive_dict) > x:
        # garder les x meilleurs sans trier toute la liste
        top_items = sorted(archive_dict.items(), key=lambda t: t[1][0], reverse=True)[:x]
        archive_dict = dict(top_items)

...
def create_population():
    return [Individual(data=data,lvl_min=LEVEL_MIN,lvl_max=LEVEL_MAX) for _ in tqdm(range(POP_SIZE),desc="creation population")]
...
def genetic_algorithm():
    ...
    print("2")
    for generation in tqdm(range(GENERATIONS),desc = f"Génération:"):
        new_population = []

        selected_individuals = sus_selection(population,8)
        for _ in tqdm(range(POP_SIZE),desc = "crossover : ") :

...
def get_recipe_id(item,lvl):
    with open("recipes_clean.json", "r") as f:
        recipes = json.load(f)
    for recipe in recipes["recipes"]:
        if recipe["type"] == item.upper() and recipe["lvl"]["minimum"] == lvl:

            return recipe["id"]
    raise ValueError("item or lvl not found in recipes_clean")
...
def remove_useless_ingredients(stat,item,ingredients):
    if item in ["chestplate",
                    "helmet",
                    "leggins",
                    "boots",
        ...
        return [
            ing for ing in ingredients
            if (stat in ing["ids"].keys()) or (ing["itemIDs"]["dura"]>0) or (any(x > 0 for x in (ing["itemIDs"].values()) ))
        ]
    elif item in ["potion",
                    "scroll",
                    "food"]:
        return [
            ing for ing in ingredients
            if (stat in ing["ids"].keys()) or (ing["consumableIDs"]["dura"]>0) or (any(x > 0 for x in (ing["itemIDs"].values()) ))
        ]
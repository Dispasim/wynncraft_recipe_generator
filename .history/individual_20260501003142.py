...
recipes = pd.read_excel(file, sheet_name=None)


class Individual:
    def __init__(self,data,item="scroll",chosen_ingredients = None,lvl_max = 120,path="Recipes.xlsx"):
        ...
        self.raw_recipe_df = self.import_recipe_df(path)
                ...
                if "identifications" in self.recipe[i][j]:
                    if stat in self.recipe[i][j]["identifications"]:
                        if min_max_or_mean == "mean":
                            ...
                            self.recipe[i][j]["identifications"][stat]["max"]
                        elif   min_max_or_mean == "raw": 
                            self.recipe[i][j]["identifications"][stat]["raw"]   
                        else:
    ...
    def recalculate_duration(self):    
        self.duration = self.base_duration + sum(ing["consumableOnlyIDs"]["duration"] for ing in self.recipe.flat) 

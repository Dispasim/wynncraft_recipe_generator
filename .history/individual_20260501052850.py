            ...
            for j in range(2):
                ...
                if "identifications" in temp_ing.keys():
                    for id in temp_ing["identifications"].keys():
                        ...
                        stats_dic[id]["min"] = stats_dic[id]["min"] + (temp_ing["identifications"][id]["min"] * (1 + (temp_boost_array[i,j]/100))) 
                        stats_dic[id]["max"] = stats_dic[id]["max"] + (temp_ing["identifications"][id]["max"] * (1 + (temp_boost_array[i,j]/100))) 
        for id in stats_dic.keys():
            print(f"{id} : {int(stats_dic[id]["min"])} - {int(stats_dic[id]["max"])}")            

...
def test_page():
    ...
    if st.button("generate"):
        ind = Individual(data=data,recipes_df = import_recipe_df("ressources/Recipes.xlsx",item),item = item,chosen_ingredients=[ingredient_1,ingredient_2,ingredient_3,ingredient_4,ingredient_5,ingredient_6],lvl_max=level,ingredient_quality_coefficient=calculate_ingredient_quality_bonus(item,quality_one,quality_two))
        st.write("Best fitness:", ind.fitness(stat=translated_stat,duration_min=duration_min,min_max_or_mean=st.session_state.min_max_or_mean,req_stats=st.session_state.req_stats))

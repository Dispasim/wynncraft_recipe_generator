...
with st.sidebar:
    ...
    item = st.selectbox("Item", ["scroll","chestplate",
                    ...
                    "potion",
                    "food"])
    #stat = st.selectbox(    "Stat",options=stats_list,index=stats_list.index("Gather XP Bonus") if "Gather XP Bonus" in stats_list else 0)
    ...
    stranslated_stats = [stats_data[stat] for stat in stats]
...
def main_page():
    ...
    if run:
        ...
        data = remove_useless_ingredients(data,stranslated_stats,item)
        ...
        trash = """for h, (fit, ind) in best_archive.items():

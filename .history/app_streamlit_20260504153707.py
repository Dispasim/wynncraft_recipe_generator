...
def main_page():
    ...
    if run:
        ...
        for gen_idx in range(generations):
            # 🔹 Ton algo génétique
            selected = select(population=population,translated_stat=translated_stat,duration_min=duration_min,min_max_or_mean = min_max_or_mean, req_stats=req_stats,method=method,n_select=n_select)
            new_pop = []
            ...
            best = max([best] + population, key=lambda ind: ind.fitness(stat=translated_stat, duration_min=duration_min, min_max_or_mean=min_max_or_mean, req_stats = req_stats))

            status_text.text(f"Génération {gen_idx + 1}/{generations}\nBest fitness = {best.fitness(stat=translated_stat, duration_min=duration_min, min_max_or_mean=min_max_or_mean, req_stats = req_stats)}")
        ...
        st.write("Best fitness:", best.fitness(stat=translated_stat, duration_min=duration_min, min_max_or_mean=min_max_or_mean, req_stats = req_stats))
        ...
        st.iframe(f"https://wynnbuilder-beta.github.io/crafter/#{best.encode_wynn_craft_hash([quality_one,quality_two])}", height=1700)
